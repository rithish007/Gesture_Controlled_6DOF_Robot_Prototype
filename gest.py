...
# Number of frames to use for the moving average
MOVING_AVERAGE_WINDOW = 5

# Initialize an array to store hand landmarks for the moving average
hand_landmarks_buffer = []
last_hand_detected_time = time.time()

# Function to update the hand landmarks buffer
def update_hand_landmarks_buffer(hand_landmarks):
    hand_landmarks_buffer.append(hand_landmarks)
    if len(hand_landmarks_buffer) > MOVING_AVERAGE_WINDOW:
        hand_landmarks_buffer.pop(0)

# Function to calculate the average hand landmarks from the buffer
def calculate_average_hand_landmarks():
    num_frames = len(hand_landmarks_buffer)
    if num_frames == 0:
        return None

    avg_hand_landmarks = mp.solutions.hands.HandLandmark()
    for hand_landmarks in hand_landmarks_buffer:
        for landmark in avg_hand_landmarks:
            landmark_point = hand_landmarks.landmark[landmark]
            avg_hand_landmarks[landmark].x += landmark_point.x
            avg_hand_landmarks[landmark].y += landmark_point.y
            avg_hand_landmarks[landmark].z += landmark_point.z

    for landmark in avg_hand_landmarks:
        avg_hand_landmarks[landmark].x /= num_frames
        avg_hand_landmarks[landmark].y /= num_frames
        avg_hand_landmarks[landmark].z /= num_frames

    return avg_hand_landmarks

def no_hand_detected():
    global last_hand_detected_time
    return time.time() - last_hand_detected_time >= 5
...
def landmark_to_servo_angle(hand_landmarks):
    servo_angle = [180, 180, 60, 60, 90, 90]  # [0, 1, 2, 3, 4, 5]
    WRIST = hand_landmarks.landmark[0]
...
with mp_hands.Hands(model_complexity=0, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            last_hand_detected_time = time.time()  # Update the last detected time
            if len(results.multi_hand_landmarks) == 1:
                hand_landmarks = results.multi_hand_landmarks[0]

                # Update hand landmarks buffer
                update_hand_landmarks_buffer(hand_landmarks)

                # Calculate average hand landmarks
                avg_hand_landmarks = calculate_average_hand_landmarks()

                if avg_hand_landmarks:
                    # Use avg_hand_landmarks for servo control
                    servo_angle = landmark_to_servo_angle(avg_hand_landmarks)
        ...
        if no_hand_detected():
            rotateServo(pins, [180, 180, 60, -20, 85, 85])
